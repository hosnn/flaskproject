...
def index():
  ...
  return render_template("detector/index.html", user_images=user_images, user_image_tag_dict=user_image_tag_dict, csrf_token=csrf_token, delete_form=delete_form)
...
def detect(image_id):
    ...
    # 감지된 물체가 없을 경우 처리
    if not tags:  # 태그가 비어 있으면 감지된 물체가 없다는 의미
        flash("감지된 물체가 없습니다.")
        return redirect(url_for("detector.index"))
